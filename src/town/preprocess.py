...
RAW_DATA_FILE = INTERIM_DATA_DIR / "train_set_Town.csv"
TARGET_COLUMN = TARGET2
MODEL_NAME = "dccuchile/bert-base-spanish-wwm-cased"

PROCESSED_DATA_DIR = PREPROCESSED_DATA_DIR / "town"
...
def preprocesar():
    ...
    town2id = {town: idx for idx, town in enumerate(df[TARGET_COLUMN].unique())}
    id2town = {idx: town for town, idx in town2id.items()}
    df['label'] = df[TARGET_COLUMN].map(town2id)

    # Crear el directorio de salida si no existe
    PROCESSED_DATA_DIR.mkdir(parents=True, exist_ok=True)
    
    # Guardar mapeo para usarlo en inferencia o análisis posterior
    path_mapeo = PROCESSED_DATA_DIR / "id2town.json"
    with open(path_mapeo, 'w', encoding='utf-8') as f:
        json.dump(id2town, f, ensure_ascii=False, indent=4)
